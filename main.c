...
struct vocab_word {
    ...
    int ids[200]; // This part is added to save the sub-words
};

//Throughout the code, everywhere there is a need for a word embedding, the embedding is constructed using its
// sub-word embeddings. At first the sub-words are genrated using the function, but then they are saved in the vocab
//structure and retrived where ever they are needed.
...
int binary = 0, cbow = 1, debug_mode = 2, window = 48, min_count = 5, num_threads = 8, min_reduce = 1;
int *vocab_hash;
...
long long train_words = 0, word_count_actual = 0, iter = 15, file_size = 0, classes = 0, number_type=0;
int nosel = 0;
int seldist = 0;
...
void InitUnigramTable() {
    ...
    double train_words_real_pow = 0;
    double d1, dreal, power = 0.75;
    int words[number_type];
    table = (int *)malloc(table_size * sizeof(int));
    table_words = (int *)malloc(table_size * sizeof(int));
    for (a = 0; a < vocab_size; a++) {

        if (vocab[a].word[0] == '<'){
            words[first] = a;
            first++;
            train_words_real_pow += pow(vocab[a].cn, power);
        }
    ...
    // This is to separate the table of words from subwords for negative sampling. to reduce the amount of misses.
    i = 0;
    dreal = pow(vocab[words[0]].cn,power) / train_words_real_pow;
    for (a = 0; a < table_size; a++) {
        table_words[a] = words[i];
        if (a / (double)table_size > dreal) {
            i++;
            dreal += pow(vocab[words[i]].cn, power) / train_words_real_pow;
        }
        if (i >= number_type) i = number_type - 1;
    }
}
...
void append1(char* s, char c, int index) {
    s[index] = c;
    s[index+1] = '\0';
}

// this function generate the subwords for each word.
int GetSubWords(char *word, char subwords[NUM_STR][MAX_STRING]){
    if (strcmp(word, "</s>") == 0) {
        strcpy(subwords[0], "</s>");
        return 1;
    }
    char copyWord[MAX_STRING+2]={};
    char word2[MAX_STRING+2]={};
    ///----------adding the whole word
    append1(copyWord, '<', 0);
    strcat(copyWord, word);
    append(copyWord,'>');
    strcpy(subwords[0],copyWord);
    //--------- adding < and >
    ...
    int counter = 1 ;
    char ext[11];
    for (size_t i=3; i<strlen(word2);i++){
        char ngram[10];
        memset(ngram, 0, sizeof(ngram));
        memset(ext, 0, sizeof(ext));
        //add the language prefix to each subword
        ...
        if ((word2[i] & 0xC0) == 0x80){
            continue;
        }
        for (size_t j=i, n=4; j<strlen(word2) && n<= 9; n++){
            append(ext, word2[j++]);
            while (j<strlen(word2) && (word2[j] & 0xC0) == 0x80){
                append(ext,word2[j++]);
            }
            if (n>=6 && !(n==4 && (i==3 || j== strlen(word2)))){

                strcpy(subwords[counter],ext);
                counter++;
                if (counter>=200){
                    return counter;
                }
...
// this function generate the subwords for each word during trainin.
int GetSubWordsTraining(char *word, char subwords[NUM_STR][MAX_STRING]){
    if (strcmp(word, "</s>") == 0) {
        strcpy(subwords[0], "</s>");
        return 1;
    }
    char copyWord2[MAX_STRING+2]={};
    char copyWord[MAX_STRING+2]={};
    strcpy(copyWord2, word);
    memmove(&copyWord2[0], &copyWord2[0 + 1], strlen(copyWord2) - 0);
    strncpy(copyWord,copyWord2,3);
    strcat(copyWord,"<");
    strcat(copyWord,copyWord2+3);
    //copyWord2[strlen(word)-1] = 0;

    strcpy(subwords[0],word);
    int counter = 1 ;
    char ext[11];
    for (size_t i=3; i<strlen(copyWord);i++){
        char ngram[10];
        memset(ngram, 0, sizeof(ngram));
        memset(ext, 0, sizeof(ext));
        if (word[1]=='e' && word[2]=='n'){
            ext[0]='e';
            ext[1]='n';
            ext[2]='_';
        } else {
        ...
        if ((copyWord[i] & 0xC0) == 0x80){
            continue;
        }
        for (size_t j=i, n=4; j<strlen(copyWord) && n<= 9; n++){
            append(ext, copyWord[j++]);
            while (j<strlen(copyWord) && (copyWord[j] & 0xC0) == 0x80){
                append(ext,copyWord[j++]);
            }
            if (n>=6 && !(n==4 && (i==3 || j== strlen(copyWord)))){
                strcpy(subwords[counter],ext);
                counter++;
...
// Get the subwords for a word and store them in the structure.
void AssingSubwords1() {

    for (size_t  i = 1; i< vocab_size; i++){
        char *token = vocab[i].word;
        if (token[0] == '<') {
            char subwords[NUM_STR][MAX_STRING]={};
            int counter = GetSubWordsTraining(token, subwords);
            vocab[i].ids[0] = counter;
            for (size_t j = 1 ; j<=counter ; j++){
               // printf("%s\n", subwords[j-1]);
                vocab[i].ids[j] =  SearchVocab(subwords[j-1]);
...
void LearnVocabFromTrainFile() {
    ...
    while (1) {
        ...
        char subwords[NUM_STR][MAX_STRING]={};
        int counter = GetSubWords(word, subwords);
        //adding them to vocabulary
        int word_id;
        for (size_t j=0; j<counter;j++) {
            //i = SearchVocab(word);
            i = SearchVocab(subwords[j]);
            if (i == -1) {
                a = AddWordToVocab(subwords[j]);
		if (a == -10) break;
                vocab[a].cn = 1;
            } else vocab[i].cn++;
        }
...
int SearchVocabDic(char *word) {
    char copyWord[MAX_STRING+2]={};
    if (strlen(word) > MAX_STRING)
        return -1;
    append1(copyWord, '<', 0);
    strcat(copyWord, word);
    append(copyWord,'>');
    unsigned int hash = GetWordHash(copyWord);
    while (1) {
        if (vocab_hash[hash] == -1) return -1;
        if (!strcmp(copyWord, vocab[vocab_hash[hash]].word)) return vocab_hash[hash];
        hash = (hash + 1) % vocab_hash_size;
    }
    return -1;
}
...
void ReadDict(){
    ...
    while (fgets(buf,1000,fin) != NULL){
        ...
        int en_id = SearchVocabDic(tokens[0]);
        int it_id = SearchVocabDic(tokens[1]);
        if (en_id != -1) en_vocab ++;
        if (it_id != -1) it_vocab ++;
...
//find the best translation to replace the center word based on the context.
real calculate_similarity_context(real* context, int word2){
    float total_1=0, total_2=0, total=0;
    int i;
    int counter = vocab[word2].ids[0]; //GetSubWordsTraining(vocab[word].word, wordsubword);
    real *sumofwordSubwords = (real *) calloc(layer1_size, sizeof(real));
    for (size_t i = 1; i <= counter; i++) {
        int subwordId = vocab[word2].ids[i]; //SearchVocab(wordsubword[i]);
        if (subwordId == -1) {
            printf("similarity\n");
            exit(987);
        }
        for (size_t c = 0; c < layer1_size; c++)
            sumofwordSubwords[c] += syn0[c + subwordId * layer1_size];
    }
    for (size_t c = 0; c < layer1_size; c++) sumofwordSubwords[c] /= counter;

    for (i = 0; i < layer1_size; i++){
        real w1 = context[i];
        real w2 = sumofwordSubwords[i]; //syn0[word2 * layer1_size + i];
        total_1 += w1*w1;
        total_2 += w2*w2;
        total += w1 * w2;
    }
...
//this part is not modified to use sub-words.
...
//get the context and find the place that this thread should work on

void *TrainModelThread(void *id) {
    long long a, b, d, cw, word, last_word, sentence_length = 0, sentence_position = 0;
    long long word_count = 0,numbtoken=0 , last_word_count = 0, sen[MAX_SENTENCE_LENGTH + 1];
    ...
    while (1) {
        ...
        if (sentence_length == 0) {
            ...
            while (1) {
                ...
                //---------------------------Ali change
                char copyWord1[MAX_STRING+2]={};
                char copyWord[MAX_STRING+2]={};
                ReadWord(copyWord1, fi);
                if (feof(fi))  break;
                //word = SearchVocab(copyWord);
                append1(copyWord, '<', 0);
                strcat(copyWord, copyWord1);
                append(copyWord,'>');
                //printf("%s\n",copyWord);
                int check = SearchVocab(copyWord);
                if (check == -1)  continue;
                word = check;
                word_count++;
                ...
                if (sample > 0) {
                    real ran = (sqrt(vocab[word].cn / (sample * train_words)) + 1) * (sample * train_words) / vocab[word].cn; //(sqrt(vocab[word].cn / (sample * train_words)) + 1) * (sample * train_words) / vocab[word].cn;
                    next_random = next_random * (unsigned long long)25214903917 + 11;
                ...
                sentence_length++;
                // Find out what is inside sen?
                if (sentence_length >= MAX_SENTENCE_LENGTH) { //printf("break sentence\n");
                break;}
        ...
        // So that randomly sample the windows b
        ...
        if (cbow) {  //train the cbow architecture
            ...
            for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {// if not the middle word
                    ...
                    for (size_t i=1; i<=counter;i++) {
                        ...
                        for (size_t c=0; c<layer1_size; c++)   sumSubwords[c] += syn0[c+subwordId*layer1_size];
                    }

                    for (size_t c=0;c<layer1_size;c++) sumSubwords[c] /= counter;
            ...
            if (cw) {
                ...
                if (negative > 0) for (d = 0; d < negative + no_positive + no_combine; d++) {
                        ...
                        } else if (d < no_positive + no_combine) {
                        ...
                        //-----------------------------------------------------------Ali: check the existence of word
                       ...
                       // l2 = target * layer1_size;// l2 is the position in the embedding array where target is embedded
                        ...
                        for (size_t i = 1; i <= counter; i++) {
                            ...
                            for (size_t c = 0; c < layer1_size; c++) {
                                sumofNegtargetSubwords[c] += syn1neg[c + subwordId * layer1_size];
                                sumofPostargetSubwords[c] += syn0[c + subwordId * layer1_size];
                            }

                        }
                        for (size_t c = 0; c < layer1_size; c++) {
                            sumofNegtargetSubwords[c] /= counter;
                        ...
                        for (size_t i = 1; i <= counter; i++) {
                            ...
                            for (c = 0; c < layer1_size; c++) {
                                ...
                                if (reg_sen > 0) {
                                    ...
                                    syn0[c + subwordId * layer1_size] += 2 * alpha * reg_sen * (save_value - save_value1);
                                }
                            }
                        }
                        free(sumofNegtargetSubwords);
                        free(sumofPostargetSubwords);
                }
                // hidden -> in
                for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
                        ...
                        for (size_t i = 1; i <= counter1; i++) {
                            ...
                            //-------------------------
                            for (c = 0; c < layer1_size; c++) syn0[c + subwordId * layer1_size] += neu1e[c];
                        }
                }
            }
...
//Setting all the arguments and call the train model
int main(int argc, char **argv) {
    ...
    //strcpy(train_file,"/Users/cons13411/CLionProjects/extendedfasttext/data/mono/en_it.shuf.10k");
    if ((i = ArgPos((char *)"-dict", argc, argv)) > 0) strcpy( dict_file, argv[i + 1]);
    //strcpy(dict_file,"/Users/cons13411/CLionProjects/extendedfasttext/data/dicts/en.it.panlex.all.processed");
    ...
    //strcpy(save_vocab_file,"/Users/cons13411/CLionProjects/extendedfasttext/data/vocab.txt");
    ...
    //strcpy(output_file,"/Users/cons13411/CLionProjects/extendedfasttext/data/output.vec");
    if ((i = ArgPos((char *)"-outputn", argc, argv)) > 0) strcpy(output_file_neg, argv[i + 1]);
    //strcpy(output_file_neg,"/Users/cons13411/CLionProjects/extendedfasttext/data/outputn.vec");
